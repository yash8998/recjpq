...
class BPRWithOneHotAssignmentStrategyPCA(CentroidAssignmentStragety):
    ...
    def get_one_hot_encoding(self, genres):
        encoder = OneHotEncoder(sparse=False)
        genre_encoded = encoder.fit_transform(np.array(genres).reshape(-1, 1))
        return genre_encoded.sum(axis=0)
    ...
    def assign(self, train_users):
        ...
        for item_id in unique_items:
            ...
            genre_emb = self.get_one_hot_encoding(genres)
            # Store embedding at index corresponding to item_id
            genre_embeddings[item_id, :len(genre_emb)] = genre_emb
        ...
        pca = PCA(n_components=item_code_bytes/2)
        ...
        for i in range(self.item_code_bytes):
            ...
            ith_component = combined_embeddings[:, i]
