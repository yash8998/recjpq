...
class BPRWithContextSegregationAssignmentStrategy(CentroidAssignmentStragety):
    ...
    def assign(self, train_users):
        ...
        item_assignments = []
        for i in range(self.item_code_bytes//2):
            ...
            item_assignments.append(component_assignments)
        ...
        for i in range(self.item_code_bytes//2):
            ...
            component_assignments = discretizer.fit_transform(ith_component).astype('uint8')[:, 0]
            item_assignments.append(component_assignments)
        return np.transpose(np.array(item_assignments))
